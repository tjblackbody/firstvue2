<template>
    ...
    <main>
      <div class="content">
        <div class="l">
          ...
          <ul>
            <li v-for="item,index in productInfo.imgAltas" :key="index"
        ...
        <div class="r">
          <div class="top">
            ...
            <div class="score">积分：{{productInfo.coin}}</div>
          </div>
          <div class="down">
            <section v-for="item,index in productInfo.parameterJson" :key="index">
              <strong>选择{{ item.title }}</strong>
              <ul>
                <li :class="val.currentActivate?'active':''" v-for="val in item.parameters" :key="val.id"
                @click="tiaozhaunluyou(val.id)">
                  {{ val.title }}
                </li>
            ...
            <div class="btns">
              <div class="addToCart" @click="ShopcarAPI">加入购物车</div>
      ...
      <aside>
        <h3>你还可以兑换</h3>
        <ul>
          <li v-for="item in themYouCanBuy" :key="item.id" @click="tiaozhaunluyou(item.id)">
            <div class="l">
    ...
    <ul class="tabs">
        <li :class="xianshi===true?'active': '' " @click="xiangqingxianshi" >
            礼品详情
        </li>
        <li :class="xianshi===false?'active': '' "   @click="xiangqingxianshi2">
            常见问题
        </li>
    </ul>
    <div v-show="xianshi" v-html="productInfo.description">

    </div>
    <div class="issue" v-show="!xianshi">
...
import { GoodDetailsAPI, shopcarAPI } from '@/request/Api'
...
export default {
  ...
  methods: {
    ...
    tiaozhaunluyou (id) {
      this.$router.push(`/Dataxiangqing?id=${id}`)
      this.getxiangqing(id)
    },
    async getxiangqing (goodId) {
      const res = await GoodDetailsAPI(goodId)
      if (!res) return
